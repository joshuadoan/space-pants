...
// TreasureCollector-specific color palette - treasure/valuable themed
const TREASURE_COLORS = [
  ...
  Color.fromHex("#DAA520"), // Goldenrod
  Color.fromHex("#B8860B"), // Dark goldenrod
  Color.fromHex("#C9B037"), // Brass
  Color.fromHex("#E6C200"), // Bright gold
  Color.fromHex("#F4A460"), // Sandy brown
  Color.fromHex("#CD853F"), // Peru
  Color.fromHex("#D4AF37"), // Metallic gold
  Color.fromHex("#FFC125"), // Golden yellow
  Color.fromHex("#FFD700"), // Pure gold
];

export function createTreasureCollectorShipOutOfShapes(): GraphicsGroup {
  const blocks: { rect: Rectangle; offset: Vector }[] = [];
  
  // Generate a random seed for this treasure collector design
  const seed = Math.random();
  
  // Select random colors from treasure palette
  const primaryColor = TREASURE_COLORS[Math.floor(Math.random() * TREASURE_COLORS.length)];
  const secondaryColor = TREASURE_COLORS[Math.floor(Math.random() * TREASURE_COLORS.length)];
  const accentColor = TREASURE_COLORS[Math.floor(Math.random() * TREASURE_COLORS.length)];
  
  // Treasure collector design patterns - treasure hunting themed
  const pattern = Math.floor(seed * 4); // 4 different patterns
  
  switch (pattern) {
    case 0: // Treasure hunter - sleek with treasure chest
      blocks.push({ rect: new Rectangle({ width: 12, height: 8, color: primaryColor }), offset: new Vector(0, 0) }); // Main body
      blocks.push({ rect: new Rectangle({ width: 6, height: 5, color: accentColor }), offset: new Vector(0, 4) }); // Treasure chest
      blocks.push({ rect: new Rectangle({ width: 4, height: 3, color: secondaryColor }), offset: new Vector(-7, -2) }); // Left scanner
      blocks.push({ rect: new Rectangle({ width: 4, height: 3, color: secondaryColor }), offset: new Vector(7, -2) }); // Right scanner
      blocks.push({ rect: new Rectangle({ width: 5, height: 2, color: accentColor }), offset: new Vector(0, -6) }); // Detector
      break;
      
    case 1: // Collector ship - wide with multiple storage
      blocks.push({ rect: new Rectangle({ width: 14, height: 6, color: primaryColor }), offset: new Vector(0, 0) }); // Main body
      blocks.push({ rect: new Rectangle({ width: 3, height: 4, color: accentColor }), offset: new Vector(-8, 2) }); // Left vault
      blocks.push({ rect: new Rectangle({ width: 3, height: 4, color: accentColor }), offset: new Vector(8, 2) }); // Right vault
      blocks.push({ rect: new Rectangle({ width: 4, height: 3, color: secondaryColor }), offset: new Vector(0, -5) }); // Top detector
      blocks.push({ rect: new Rectangle({ width: 6, height: 2, color: accentColor }), offset: new Vector(0, 5) }); // Engine
      break;
      
    case 2: // Compact treasure seeker - vertical with gem storage
      blocks.push({ rect: new Rectangle({ width: 10, height: 10, color: primaryColor }), offset: new Vector(0, -1) }); // Main body
      blocks.push({ rect: new Rectangle({ width: 3, height: 3, color: accentColor }), offset: new Vector(0, -7) }); // Top gem
      blocks.push({ rect: new Rectangle({ width: 4, height: 5, color: secondaryColor }), offset: new Vector(-6, 1) }); // Left storage
      blocks.push({ rect: new Rectangle({ width: 4, height: 5, color: secondaryColor }), offset: new Vector(6, 1) }); // Right storage
      blocks.push({ rect: new Rectangle({ width: 8, height: 2, color: accentColor }), offset: new Vector(0, 6) }); // Engine
      break;
      
    case 3: // Luxury collector - ornate design with side treasures
      blocks.push({ rect: new Rectangle({ width: 8, height: 8, color: primaryColor }), offset: new Vector(0, -1) }); // Body
      blocks.push({ rect: new Rectangle({ width: 3, height: 4, color: accentColor }), offset: new Vector(-6, 0) }); // Left treasure
      blocks.push({ rect: new Rectangle({ width: 3, height: 4, color: accentColor }), offset: new Vector(6, 0) }); // Right treasure
      blocks.push({ rect: new Rectangle({ width: 4, height: 3, color: secondaryColor }), offset: new Vector(0, -6) }); // Top crown
      blocks.push({ rect: new Rectangle({ width: 6, height: 2, color: accentColor }), offset: new Vector(0, 5) }); // Engine
      break;
  }
