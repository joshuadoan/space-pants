...
export class SpaceDance extends Meeple {
  ...
  constructor(position: Vector, name: string, productType?: Products) {
    ...
    this.type = MeepleType.SpaceDance;
  ...
  transaction(good: GoodType, quantity: number, transactionType: "add" | "remove") {
    switch (transactionType) {
      case "add":
        // SpaceDance buys goods (adds to inventory) - not typically used
        this.goods[good] = (this.goods[good] ?? 0) + quantity;
        break;
      case "remove":
        // SpaceDance sells goods (subtracts from inventory)
