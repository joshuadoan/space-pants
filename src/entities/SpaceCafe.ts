...
export class SpaceCafe extends Meeple {
  ...
  constructor(position: Vector, name: string, productType?: Products) {
    ...
    this.type = MeepleType.SpaceCafe;
  ...
  transaction(good: GoodType, quantity: number, transactionType: "add" | "remove") {
    switch (transactionType) {
      case "add":
        // SpaceCafe buys goods (adds to inventory) - not typically used
        this.goods[good] = (this.goods[good] ?? 0) + quantity;
        break;
      case "remove":
        // SpaceCafe sells goods (subtracts from inventory)
